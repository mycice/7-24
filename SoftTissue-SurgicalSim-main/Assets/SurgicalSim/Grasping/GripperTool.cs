...
// Runs independently from the soft-body collision solver.
...
namespace SurgicalSim.Grasping
{
    public class GripperTool : MonoBehaviour
    {
        ...
        // 视觉 GameObjects
        [Header("场景模型对象")]
        [Tooltip("场景中预先创建的上夹爪子物体。")]
        public GameObject upperJawObject;

        [Tooltip("场景中预先创建的下夹爪子物体。")]
        public GameObject lowerJawObject;

        [Tooltip("场景中预先创建的夹爪杆身子物体。")]
            ...
            out bool ownsObject)
        {
            ...
            if (go == null)
            {
                Debug.LogError($"[GripperTool] 未指定场景中的 {name}。", this);
                return null;
            }
            ...
            if (mf == null || mr == null)
            {
                Debug.LogError($"[GripperTool] {name} 必须预先挂载 MeshFilter 和 MeshRenderer。", go);
                return go;
            }
            mr.material = mat;
