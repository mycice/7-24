...
namespace ReconGridDC.Stage1TetPhysics.Physics
{
    public class XPBDSolverGPU : IDisposable
    {
        ...
        // Stage 1 deliberately keeps all tool contact disabled. Later stages can opt in.
        public bool ToolContactEnabled = false;
        ...
        public void Step(float dt)
        {
            ...
            bool useToolContact = ToolContactEnabled && _activeToolCapsules > 0;
            if (useToolContact)
            {
            ...
            for (int sub = 0; sub < NumSubSteps; sub++)
            {
                ...
                if (useToolContact && !sampledToolMetrics)
                {
        ...
        {
            if (!ToolContactEnabled || _activeToolCapsules <= 0 ||
        ...
        // The Stage 2 bridge uses this after CPU broad-phase culling to avoid
        // dispatching the expensive contact/coupling passes while the tool is away.
        public bool HasToolContactCandidates =>
            _toolCandidatesValid && _activeToolSurfaceCandidateTris > 0;

        public int ActiveToolContactCandidateCount => _activeToolSurfaceCandidateTris;
